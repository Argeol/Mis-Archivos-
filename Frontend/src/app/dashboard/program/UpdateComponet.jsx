...
export default function UpdateProgram({ id }) {
    ...
    const { data: programData, isLoading: isLoadingProgram } = useQuery({
        queryKey: ["program", id],
        queryFn: async () => {
            const response = await axiosInstance.get(`/api/Program/ProgramGetId${id}`);
    ...
    useEffect(() => {
        if (programData) {
            ...
            const matchingArea = areas.find(area => area.area_Name === programData.area_Name);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        console.log("📤 Enviando datos:", { program_Name: programName, area_Id: Number(areaId) });
        updateMutation.mutate({ program_Name: programName, area_Id: Number(areaId) });
    };
